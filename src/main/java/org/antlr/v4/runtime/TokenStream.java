...
public interface TokenStream extends IntStream {
  /**
   * Get the {@link Token} instance associated with the value returned by {@link #LA LA(k)}. This method has the same
   * pre- and post-conditions as {@link IntStream#LA}. In addition, when the preconditions of this method are met, the
   * return value is non-null and the value of {@code LT(k).getType()==LA(k)}.
   *
   * @see IntStream#LA
   */
   ...
   * {@link IntStream#seek}. If the behavior of {@code seek(index)} is unspecified for the current state and given
   * {@code index}, then the behavior of this method is also unspecified.</p>
   *
   * <p>The symbol referred to by {@code index} differs from {@code seek()} only
   * in the case of filtering streams where {@code index} lies before the end of the stream. Unlike {@code seek()}, this
   * method does not adjust {@code index} to point to a non-ignored symbol.</p>
   *
  ...
  /**
   * Return the text of all tokens within the specified {@code interval}. This method behaves like the following code
   * (including potential exceptions for violating preconditions of {@link #get}, but may be optimized by the specific
   * implementation.
   *
   * <pre>
   * TokenStream stream = ...;
  ...
  /**
   * Return the text of all tokens in the stream. This method behaves like the following code, including potential
   * exceptions from the calls to {@link IntStream#size} and {@link #getText(Interval)}, but may be optimized by the
  ...
  /**
   * Return the text of all tokens in the source interval of the specified context. This method behaves like the
   * following code, including potential exceptions from the call to {@link #getText(Interval)}, but may be optimized by
   ...
   * this stream, or if the {@code stop} occurred before the {@code start} token, the behavior is unspecified.</p>
   *
   * <p>For streams which ensure that the {@link Token#getTokenIndex} method is
   * accurate for all of its provided tokens, this method behaves like the following code. Other streams may implement
   * this method in other ways provided the behavior is consistent with this at a high level.</p>
   *
   * <pre>
   * TokenStream stream = ...;
   * String text = "";
