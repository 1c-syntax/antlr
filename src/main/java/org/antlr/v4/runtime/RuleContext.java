 ...
 * <p>
 * We form a stack of these context objects using the parent pointer. A parent pointer of null indicates that the
 ...
 * <p>
 * The invokingState fields from a context object to the root together form a stack of rule indication states where the
 * root (bottom of the stack) has a -1 sentinel value. If we invoke start symbol s then call r1, which calls r2, the
 * would look like this:
 * <p>
 * SContext[-1]   - root node (bottom of the stack) R1Context[p]   - p in rule s called r1 R2Context[q]   - q in rule r1
 * called r2
 * <p>
 * So the top of the stack, _ctx, represents a call to the current rule and it holds the return address from another
 * rule that invoke to this rule. To invoke a rule, we must always have a current context.
 * <p>
 * The parent contexts are useful for computing lookahead sets and getting error information.
 * <p>
 * These objects are used during parsing and prediction. For the special case of parsers, we use the subclass
 * ParserRuleContext.
 *
...
public class RuleContext implements RuleNode {
  ...
  /**
   * Return the combined text of all child nodes. This method only considers tokens which have been added to the parse
   * tree.
   * <p>
   * Since tokens on hidden channels (e.g. whitespace or comments) are not added to the parse trees, they will not
   * appear in the output of this method.
   */
  @Override
  ...
  /**
   * For rule associated with this parse tree internal node, return the outer alternative number used to match the
   * input. Default implementation does not compute nor store this alt num. Create a subclass of ParserRuleContext with
  ...
  /**
   * Set the outer alternative number for this context node. Default implementation does nothing to avoid backing field
   * overhead for trees that don't need it.  Create a subclass of ParserRuleContext with backing field and set option
   * contextSuperClass.
   *
   * @since 4.5.3
   */
  ...
  /**
   * Print out a whole tree, not just a node, in LISP format (root child1 .. childN). Print just a node if this is a
   * leaf. We have to know the recognizer so we can get rule names.
   */
  @Override
  ...
  public String toString(@Nullable List<String> ruleNames, @Nullable RuleContext stop) {
    ...
    while (p != null && p != stop) {
      if (ruleNames == null) {
        ...
        var ruleName = ruleIndex >= 0 && ruleIndex < ruleNames.size() ? ruleNames.get(ruleIndex)
