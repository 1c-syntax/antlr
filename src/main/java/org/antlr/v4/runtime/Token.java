...
public interface Token {
  ...
  /**
   * During lookahead operations, this "token" signifies we hit rule end ATN state and did not follow it despite needing
   * to.
   */
  int EPSILON = -2;
  ...
  /**
   * All tokens go to the parser (unless skip() is called in that rule) on a particular "channel".  The parser tunes to
   * a particular channel so that whitespace etc... can go to the parser on a "hidden" channel.
   */
  ...
  /**
   * Return the channel this token. Each token can arrive at the parser on a different channel, but the parser only
   * "tunes" to a single channel. The parser ignores everything not on DEFAULT_CHANNEL.
   */
  int getChannel();

  /**
   * An index from 0..n-1 of the token object in the input stream. This must be valid in order to print token streams
   * and use TokenRewriteStream.
   * <p>
