...
/**
 * The root of the ANTLR exception hierarchy. In general, ANTLR tracks just 3 kinds of errors: prediction errors, failed
 * predicate errors, and mismatched input errors. In each case, the parser knows where it is in the input, where it is
 * in the ATN, the rule invocation stack, and what kind of problem occurred.
 */
public class RecognitionException extends RuntimeException {
  ...
  /**
   * The current {@link Token} when an error occurred. Since not all streams support accessing symbols by index, we have
   * to track the {@link Token} instance itself.
   */
  ...
  /**
   * Gets the set of input symbols which could potentially follow the previously matched symbol at the time this
   * exception was thrown.
   *
   * <p>If the set of expected tokens is not known and could not be computed,
   * this method returns {@code null}.</p>
   *
   * @return The set of token types that could potentially follow the current state in the ATN, or {@code null} if the
   ...
   *
   * @return The {@link RuleContext} at the time this exception was thrown. If the context is not available, this method
   * returns {@code null}.
   */
  @Nullable
   ...
   *
   * @return The input stream which is the symbol source for the recognizer where this exception was thrown, or
